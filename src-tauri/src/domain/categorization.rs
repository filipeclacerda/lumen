...
pub fn matches_rule(rule: &CategorizationRule, input: &CategorizationInput<'_>) -> bool {
    ...
    if rule
        .account_id
        .as_ref()
        .is_some_and(|id| id != input.account_id)
    {
        return false;
    }
    let actual = if input.amount_in_cents >= 0 {
        MovementType::Income
    } else {
        MovementType::Expense
    };
    if rule.movement_type != MovementType::Any
        && rule.movement_type != MovementType::Transfer
        && rule.movement_type != actual
    {
        return false;
    }
    ...
    match rule.operator {
        ...
        RuleOperator::Regex => RegexBuilder::new(&rule.pattern)
...
pub fn first_match<'a>(
    ...
    input: &CategorizationInput<'_>,
) -> Option<&'a CategorizationRule> {
    rules
        .iter()
        .filter(|rule| matches_rule(rule, input))
...
mod tests {
    ...
    #[test]
    fn combines_conditions() {
        let mut r = rule(RuleOperator::Contains, MovementType::Expense);
        r.min_amount_in_cents = Some(1000);
        r.max_amount_in_cents = Some(5000);
        assert!(matches_rule(
            &r,
            &CategorizationInput {
                account_id: "a",
    ...
    #[test]
    fn respects_priority_and_disabled() {
        let mut later = rule(RuleOperator::Contains, MovementType::Expense);
        later.id = "later".into();
        later.priority = 20;
        ...
        assert_eq!(
            first_match(&[later.clone(), first.clone()], &input)
                .unwrap()
                .id,
            "first"
        );
        first.enabled = false;
        assert_eq!(first_match(&[later, first], &input).unwrap().id, "later");
    }
    #[test]
    fn supports_starts_with_and_regex() {
        let input = CategorizationInput {
            account_id: "a",
            normalized_description: "POSTO CENTRAL",
            amount_in_cents: -100,
        };
        let mut r = rule(RuleOperator::StartsWith, MovementType::Any);
        r.pattern = "POSTO".into();
        assert!(matches_rule(&r, &input));
        r.operator = RuleOperator::Regex;
        r.pattern = r"^POSTO\s+\w+".into();
        assert!(matches_rule(&r, &input));
        r.pattern = "(".into();
        assert!(!matches_rule(&r, &input));
    }
    #[test]
    fn generic_pix_does_not_match() {
        assert!(!matches_rule(
            &rule(RuleOperator::Contains, MovementType::Expense),
            &CategorizationInput {
                account_id: "a",
                normalized_description: "PIX EMIT.OUTRA IF",
                amount_in_cents: -100
            }
        ));
    }
