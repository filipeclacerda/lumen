...
export function SettingsPage() {
  ...
  useEffect(() => {
    if (profile) {
      setName(profile.displayName); setIncomeInCents(profile.monthlyIncomeInCents ?? null); setIncomeInputVersion(version => version + 1);
      setDay(profile.incomeDay ? String(profile.incomeDay) : ""); setGoal(profile.financialGoal)
    }
  }, [profile]);
  async function saveProfile() {
    ...
    try {
      await api.saveProfile({
        displayName: name.trim(), monthlyIncomeInCents: incomeInCents ?? undefined,
  ...
  async function backup() {
    try {
      const path = await save({ defaultPath: "lumen-backup.db", filters: [{ name: "Backup do Lumen", extensions: ["db"] }] });
      if (path) {
  ...
  return <section><header><div><p className="eyebrow">PREFERÊNCIAS</p><h1>Configurações</h1><p className="muted">Atualize seus dados de planejamento.</p></div></header>
    ...
    {updatesEnabled && <article className="panel" style={{ marginTop: 18 }}><div className="panel-title"><h2><RefreshCw size={17} /> Atualizações</h2></div>
      <p className="muted">Confira se existe uma versão nova do Lumen. Quando houver atualização, um aviso aparece no topo da tela com a opção de instalar.</p>
      <div className="data-actions">
    ...
    <article className="panel danger-zone" style={{ marginTop: 18 }}><div className="panel-title"><h2><AlertTriangle size={17} /> Zona de risco</h2></div>
      <p className="muted">Apaga permanentemente contas, transações, categorias, regras, metas, recorrências e faturas de cartão — o Lumen reinicia sozinho e volta ao estado de instalação nova. Faça um backup antes, se precisar dos dados depois.</p>
    ...
    {resetOpen && <Modal title="Apagar todos os dados?" onClose={closeReset}>
      <div className="modal-form">
        <p className="muted">Essa ação não pode ser desfeita. Todos os dados serão apagados e o Lumen reinicia automaticamente para concluir a limpeza.</p>
