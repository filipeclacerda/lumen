...
export function Onboarding({bootstrap,onFinished}:{bootstrap:AppBootstrap;onFinished:(destination:string)=>Promise<void>}){
  ...
  async function finish(){
    ...
    try{
      await api.completeOnboarding({
        ...
        monthlyIncomeInCents:income?Math.round(Number(income)*100):undefined,
        incomeDay:incomeDay?Number(incomeDay):undefined,
        financialGoal:goal,
        accountName:accountName.trim(),accountKind,
        openingBalanceInCents:!bootstrap.hasTransactions&&openingBalance?Math.round(Number(openingBalance)*100):undefined
      });
      setCompleted(true);
  ...
  return <div className="onboarding-shell"><div className="onboarding-card">
    ...
    {step===2&&<div className="onboarding-content"><div className="step-icon"><UserRound/></div><p className="eyebrow">SEU PERFIL</p><h1>Vamos nos conhecer</h1>
      ...
      <div className="form-row"><label>Renda líquida mensal <div className="money-input"><span>R$</span><input type="number" min="0" step="0.01" value={income} onChange={e=>setIncome(e.target.value)} placeholder="0,00"/></div></label>
    ...
    {step===3&&<div className="onboarding-content"><div className="step-icon"><WalletCards/></div><p className="eyebrow">PRIMEIRA CONTA</p><h1>Configure sua conta principal</h1>
      ...
      {!bootstrap.hasTransactions&&<label>Saldo inicial <div className="money-input"><span>R$</span><input type="number" step="0.01" value={openingBalance} onChange={e=>setOpeningBalance(e.target.value)} placeholder="0,00"/></div></label>}
      {error&&<p className="form-error">{error}</p>}<div className="onboarding-actions"><button className="secondary" onClick={()=>setStep(2)}>Voltar</button><button disabled={saving} onClick={finish}>{saving?"Salvando…":"Concluir cadastro"} <CheckCircle2 size={17}/></button></div>
    </div>}
  </div></div>
}
