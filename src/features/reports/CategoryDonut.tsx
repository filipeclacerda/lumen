...
export function CategoryDonut({ categories, selectedCategoryId, onSelect }: Props) {
      ...
      <ResponsiveContainer width="100%" height={240}>
        <PieChart>
          <Pie
            ...
            cursor="pointer"
            onClick={data => {
              const categoryId = (data.payload as CategoryReport | undefined)?.categoryId;
              onSelect(categoryId === selectedCategoryId ? undefined : categoryId);
            }}
          >
            {data.map((entry, index) => (
              <Cell
                ...
                opacity={!selectedCategoryId || selectedCategoryId === entry.categoryId ? 1 : 0.35}
                stroke="var(--surface)"
                strokeWidth={2}
              />
            ))}
